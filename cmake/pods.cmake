...
# when       who       what
#----------  --------- -----------
# 2015.06.05 ak        added pods_install_configs

function(pods_install_configs)
    list(GET ARGV -2 checkword)
    if(NOT checkword STREQUAL DESTINATION)
        message(FATAL_ERROR "pods_install_configs missing DESTINATION parameter")
    endif()

    list(GET ARGV -1 dest_dir)
    list(REMOVE_AT ARGV -1)
    list(REMOVE_AT ARGV -1)
    #copy the cfg to the SHARE_OUTPUT_PATH (${CMAKE_BINARY_DIR}/share)
    foreach(cfg ${ARGV})
        get_filename_component(_cfg_name ${cfg} NAME)
        configure_file(${cfg} ${SHARE_OUTPUT_PATH}/${dest_dir}/${_cfg_name} COPYONLY)
	endforeach(cfg)
	#mark them to be installed
	install(FILES ${ARGV} DESTINATION share/${dest_dir})
endfunction(pods_install_configs)
...
macro(pods_config_search_paths)
    if(NOT DEFINED __pods_setup)
        ...
        set(SHARE_OUTPUT_PATH ${CMAKE_BINARY_DIR}/share)
