...
{
    ifstream file_path("../../data/exp_gain_table_index.csv");
...
//  Config cfg(ls, s_f, s_n, AcqType::MAXVAR, 5);
    Config cfg(ls, s_f, s_n, AcqType::MAXVAR, 0.5);
    ...
    while(true) {
            ...
            while (!gpo.is_optimal()) {
                ...
                if (gpo.evaluate (x, ewg)) {
                    best_exposure = gpo.optimal_expose() * 300 ;
                    ...
                    _synth_img_t(cam_bluefox2 , next_synth_index_t, synth_img_t, true);
...
//            printf ("[ExpCtrl]\tStart to grab best images.\n");
            ...
            CvPoint str_pos = cvPoint (50, 50);
            char str[30];
            snprintf (str, sizeof str, "%d, %d", best_exposure, (int)best_gain);
            cv::Mat result;
