...
% for in_exp = 1 : abs(log(0.0005 * 20) + 4)*2 : abs(log(0.0005 * 20) + 4)*2*39  
   for in_exp = 1:91
%     c_exp = B(1)* (0.8 + 0.001*in_exp) ;
    c_exp = B(1);
    c_in = find (radiance_map_obj{1} >= c_exp);
    c_int = c_in(1);
%     n_exp = in_exp;
    atio =1;
    n_exp = B(1) + log(atio * (in_exp))- log(1) %* abs(log(0.0005 * 20) + 4)* 1.69);
...
pause(.001)
imwrite(syn_img, sprintf('../sample_pic2/syn%d.png' , in_exp ) );
